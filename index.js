...
function printUppercase(array) {
  for (const index in array) {
    console.log(String(array[index].toUpperCase()));
  }
...
function findMinMax([firstElement, ...restElements]) {
  ...
  let minIndex = firstElement;
  let maxIndex = firstElement;
...
function printStrings(array) {
  let result = array.toString().repeat(2);
  console.log(result);
...
function multiplyByIndex(arr) {
  ...
  // Проходимо через кожен елемент вхідного масиву
  for (const elem in arr) {
    let newElem = arr[elem] * elem;
    newArr = [...newArr, newElem];
  }
...
function replaceNumbers(arr) {
  ...
  for (const elem in arr) {
    if (arr[elem] > 10) {
      newArr = [...newArr, "Greater than 10"];
    } else {
      newArr = [...newArr, "Less than or equal to 10"];
    }
  }
  return newArr;
